...
namespace Server.Misc
{
	public class Strandedness
	{
		...
		private static Point2D[] m_Exclusion = new Point2D[]
			{
				new Point2D( 4880, 0   ), new Point2D( 6500,  4200 ),
				new Point2D( 150,  700 ), new Point2D( 450,   900  )
			};
		...
		// wea: added CheckLegalBounce() functions to handle destination check for secure location
		// exclusion

		public static bool CheckLegalBounce(Point3D loc)
		{
			Point2D loc2d = new Point2D();

			loc2d.X = loc.X;
			loc2d.Y = loc.Y;

			return (CheckLegalBounce(loc2d));
		}
		...
		public static bool CheckLegalBounce(Point2D loc)
		{
			...
			try
			{
				...
				for (int i = 0; i < m_Exclusion.Length - 1; i += 2)
				{
					if (loc.X >= m_Exclusion[i].X &&
							loc.Y >= m_Exclusion[i].Y &&
							loc.X <= m_Exclusion[i + 1].X &&
							loc.Y <= m_Exclusion[i + 1].Y)
					{
						bLegal = false;
						break;
					}
		...
		public static void EventSink_Login(LoginEventArgs e)
		{
			...
			{
				if (from.Region is Regions.CustomRegion && ((Regions.CustomRegion)from.Region).GetRegionControler().CaptureArea)
				{
