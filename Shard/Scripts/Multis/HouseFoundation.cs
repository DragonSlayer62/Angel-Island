...
namespace Server.Multis
{
	...
	public class HouseFoundation : BaseHouse
	{
		...
		private enum SaveFlags
		{
			...
			DesignState_Design = 0x01,
			DesignState_Backup = 0x02,
		}

		protected DesignState m_Current;			// State which is currently visible.
		protected DesignState m_Design;				// State of current design.
		protected DesignState m_Backup;				// State at last user backup.
		private Item m_SignHanger;					// Item hanging the sign.
		protected Item m_Signpost;					// Item supporting the hanger.
		protected int m_SignpostGraphic;			// ItemID number of the chosen signpost.
		...
		private SaveFlags m_flags = SaveFlags.NOP;	// what we should save or not (StaticHouses don't save all the design states)
		...
		public virtual DesignState CurrentState
		{
			get { if (m_Current == null) SetInitialState(); return m_Current; }
			set { m_Current = value; }
		}

		public virtual DesignState DesignState
		{
			get { if (m_Design == null) SetInitialState(); return m_Design; }
			set { m_Design = value; }
		}

		public virtual DesignState BackupState
		{
		...
		public static void QueryDesignDetails(NetState state, PacketReader pvSrc)
		{
			...
			if (foundation != null && from.Map == foundation.Map && from.InRange(foundation.GetWorldLocation(), 24) && from.CanSee(foundation))
			{
				...
				if ((context != null && context.Foundation == foundation) /*|| foundation is StaticHousing.StaticHouse*/)
