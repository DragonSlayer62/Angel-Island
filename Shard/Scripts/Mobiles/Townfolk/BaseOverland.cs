	...
	public class BaseOverland : BaseCreature
	{
		...
		private class MsgTimer : Timer
		{
			...
			{	// RunUO timers can be queued suchj that they will OnTick() after they are stopped.
				...
				m_mobile.DebugSay("OnTick: Restarting messages");
			}
		}
		...
		public PlayerMobile Hero(Mobile mob)
		{
			return GetPlayerMobile(mob);
		}
		...
		public virtual void OnMoongate()
		{
			if (GateTravel == false)
			{
				// after we're done reading, kick off the town crier if need be
