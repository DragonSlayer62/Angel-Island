...
namespace Server.Mobiles
{
	public class BaseHuman : BaseCreature
	{
		...
		{
			// Data/toyshop.z5
			if (!System.IO.File.Exists(String.Format("Data/{0}.z5", StoryFile)))
			{
				Console.WriteLine("Error: {0} does not exist", String.Format("Data/{0}.z5", StoryFile));
				return false;
			}
			// allocate a conversation, thread, and start it
			m_Conversations.Add(with.Serial, new Conversation(with, String.Format("Data/{0}.z5", StoryFile), isPrivate));
			return true;
		}
