...
namespace Server.Mobiles
{
	public class Auctioneer : BaseVendor
	{
		...
		{	// called once at the start of each auction
			...
			if (Misc.Diagnostics.Assert(item != null, "item == null"))
			{
		...
		{
			Item item = null;
			if (Misc.Diagnostics.Assert(Backpack != null, "Backpack == null"))
		...
		{
			PlayerMobile pm = World.FindMobile(m_HighBidMobile) as PlayerMobile;
			if (Misc.Diagnostics.Assert(pm != null && pm.Account != null && pm.Account.ToString().GetHashCode() == m_HighBidMobileHC, "Hash Code mismatch"))
				return true;
