...
namespace Server.Movement
{
	public class MovementImpl : IMovementImpl
	{
		...
		private bool Check(Map map, Mobile m, ArrayList items, int x, int y, int startTop, int startZ, bool canSwim, bool cantWalk, bool canFly, out int newZ)
		{
			newZ = 0;
			...
			for (int i = 0; i < tiles.Length; ++i)
			{
				...
				if ((flags & ImpassableSurface) == TileFlag.Surface || (canSwim && (flags & TileFlag.Wet) != 0)) // Surface && !Impassable
				{
					...
					if (stepTop >= itemTop)
					{
						...
						if (IsOk(m, ignoreDoors, ourZ, testTop, tiles, items, canFly))
						{
							newZ = ourZ;
							moveIsOk = true;
						}
			...
			if (considerLand && !landBlocks && stepTop >= landZ)
			{
				...
				if (shouldCheck && IsOk(m, ignoreDoors, ourZ, testTop, tiles, items, canFly))
				{
		...
		public bool CheckMovement(Mobile m, Map map, Point3D loc, Direction d, out int newZ)
		{
			...
			else
			{
				...
				{
					items = sectorStart.Items.Values;
				...
				{
					items = sectorForward.Items.Values;
			...
			bool moveIsOk = Check(map, m, itemsForward, xForward, yForward, startTop, startZ, m.CanSwim, m.CantWalk, m.CanFly, out newZ);

			if (moveIsOk && checkDiagonals)
			{
				int hold;

				if (!Check(map, m, itemsLeft, xLeft, yLeft, startTop, startZ, m.CanSwim, m.CantWalk, m.CanFly, out hold) && !Check(map, m, itemsRight, xRight, yRight, startTop, startZ, m.CanSwim, m.CantWalk, m.CanFly, out hold))
					moveIsOk = false;
			}
