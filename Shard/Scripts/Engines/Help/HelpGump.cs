 ...
 *      -Prevent auto-move if there are nearby items/mobs and log using LogHelper.Cheater
 *	3/2/07, Pix
...
namespace Server.Engines.Help
{
	...
	public class HelpStuckQuestion : QuestionMenu
	{
		...
		public static void Initialize()
		{
			Server.CommandSystem.Register("HelpStuck", AccessLevel.GameMaster, new CommandEventHandler(HelpStuck_OnCommand));
		}
		...
		[Usage("HelpStuck")]
		[Description("Test auto movement logic.")]
		public static void HelpStuck_OnCommand(CommandEventArgs e)
		{
			try
			{
				if (!HelpGump.TryMoveStuckPlayer(e.Mobile, 4))
					e.Mobile.SendMessage("Auto move failed.");
				else
					e.Mobile.SendMessage("Auto move ok.");
			}
			catch (Exception ex)
			{
