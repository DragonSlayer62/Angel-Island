	...
	public class Guildstone : Item
	{
		...
		public bool Peaceful
		{
			...
			set
			{
				...
				if (value == true && Guild != null)
				{
					...
					{
						this.SendMessage("You may not create a Peaceful guild that is also aligned.");
						errors++;
					}

					if (m_Guild.GuildWarRing == true)
					{
						this.SendMessage("You may not create a Peaceful guild that belongs to the war ring.");
						errors++;
					}

					if (m_Guild.Allies.Count > 0 || m_Guild.AllyDeclarations.Count > 0 || m_Guild.AllyInvitations.Count > 0)
					{
						this.SendMessage("You may not create a Peaceful guild that wishes to ally.");
						errors++;
					}

					if (m_Guild.Enemies.Count > 0 || m_Guild.WarDeclarations.Count > 0 || m_Guild.WarInvitations.Count > 0)
					{
						this.SendMessage("You may not create a Peaceful guild that wishes to war.");
						errors++;
					}
				}
		...
		public void OnPrepareMove(Mobile from)
		{
			if (from is PlayerMobile)
			{
				...
				if (MoveItemToIntStorage() && from.Backpack.CheckHold(from, deed, true, false, 0, 0))
				{
