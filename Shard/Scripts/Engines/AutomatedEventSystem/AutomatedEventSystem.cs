 ...
 *
 *      Copyright (c) 2004 Tomasello Software LLC.O
...
namespace Server
{
	...
	public class AutomatedEventSystem : Item
	{
		...
		DateTime m_AnnounceTime;					// when we make Town Crier announcments
		TimeSpan m_AnnounceDelta;					// frequency between announcements
		DateTime m_BroadcastTime;					// when we make Broadcast announcments 
	...
	public class TownInvasionAES : AutomatedEventSystem
	{
		...
		bool m_test = false;						// are we in test mode?
		...
		public override void OnEventEndInit()
		{
			...
			if (m_ChampionSpawn != null && m_ChampionSpawn.Deleted == false)
			{
				...
				m_ChampionSpawn.Monitor = null;

				// let the fun end!
				m_ChampionSpawn.Active = false;								// stop the creatures;
		...
		public override void OnAnnounce()
		{
			...
			if (EventStarted == false)
			{
				...
				switch (Utility.Random(3))
				{
					case 0:
						{
							lines[0] = String.Format("A evil swarm approaches.");
	...
	public class KinRansomAES : AutomatedEventSystem
	{
		...
		RegionControl m_RegStone = null;			// region controller of the chest
		...
		public override void OnEventStartInit()
		{
			...
			try
			{
				...
				{
					// save old modes
					m_OldCountMode = m_RegStone.NoMurderZone;
					m_OldGuardMode = m_RegStone.IsGuarded;
					...
					// set new modes
					m_RegStone.NoMurderZone = true;
					m_RegStone.IsGuarded = false;
					m_RegStone.CanUseStuckMenu = false;
		...
		public override void OnEventEndInit()
		{
			...
			{
				m_RegStone.NoMurderZone = m_OldCountMode;
				m_RegStone.IsGuarded = m_OldGuardMode;
				m_RegStone.CanUseStuckMenu = true;
	...
	{
		bool m_test = false;				// for testing only
		...
		//	this object when Cron.GameTimeNow > FinalCleanup.
