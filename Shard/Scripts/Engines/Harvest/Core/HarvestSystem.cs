	...
	public abstract class HarvestSystem
	{
		...
		public virtual void FinishHarvesting(Mobile from, Item tool, HarvestDefinition def, object toHarvest, object locked)
		{
			...
			if (skillBase >= resource.ReqSkill && from.CheckSkill(def.Skill, resource.MinSkill, resource.MaxSkill))
			{
				...
				if (type != null)
				{
					Item item = Construct(type, from, ObjectArgs(type, from, tool, def, map, loc, resource));
		...
		public virtual Item Construct(Type type, Mobile from, object[] args)
		{
		...
		public virtual object[] ObjectArgs(Type type, Mobile from, Item tool, HarvestDefinition def, Map map, Point3D loc, HarvestResource resource)
		{
			return null;
		}
