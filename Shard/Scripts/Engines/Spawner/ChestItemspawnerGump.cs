...
namespace Server.Items
{
	...
	public class ChestItemSpawnerType
	{
		...
		public static Item GetLootPack(string str)
		{
			int tmp = 0;
			int offset = 0;
			if (str.Contains("0X") || str.Contains("0x"))
				offset = 2;
			if (Int32.TryParse(str.Substring(offset), System.Globalization.NumberStyles.AllowHexSpecifier, null, out tmp))
			{
				Item item = World.FindItem((Serial)tmp);
				if (item != null)
					return item;
			}
			else if (Int32.TryParse(str, System.Globalization.NumberStyles.Integer, null, out tmp))
			{
				Item item = World.FindItem((Serial)tmp);
				if (item != null)
					return item;
