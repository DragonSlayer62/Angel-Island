...
namespace Server.Items
{
	...
	public class SensoryConsole : Item
	{
		...
		public Rectangle2D Rectangle
		{
			get
			{
				return m_region.Coords.Count > 0 ? (Rectangle2D)m_region.Coords[0] : new Rectangle2D(0, 0, 0, 0);
			}
			/*set
			{	
			 * never seems to get called. I suspect a bug in the way RunUO handles the Props for a rect
			 * We will instead the the setting of the Start and End points as the way to initialize the sensory zone
		...
		public Point2D Begin
		{
			get
			{
				return m_region.Coords.Count > 0 ? ((Rectangle2D)m_region.Coords[0]).Start : new Point2D(0, 0);
			}
			set
			{
				if (m_region.Coords.Count > 0)
				{
					Rectangle2D zone = (Rectangle2D)m_region.Coords[0];
					m_region.Coords.Clear();
					zone.Start = value;
					m_region.Coords.Add(zone);
				}
				else
				{
					Rectangle2D zone = new Rectangle2D(value, new Point2D(0, 0));
					m_region.Coords.Add(zone);
				}
		...
		public Point2D End
		{
			get
			{
				return m_region.Coords.Count > 0 ? ((Rectangle2D)m_region.Coords[0]).End : new Point2D(0, 0);
			}
			set
			{
				if (m_region.Coords.Count > 0)
				...
				{
					Rectangle2D zone = new Rectangle2D(new Point2D(0, 0), value);
					zone = FixRect(zone);
					m_region.Coords.Add(zone);
				}
		...
		public void InitRegion()
		{
			...
			m_region.InnBounds = new ArrayList();
		...
		private void UpdateRegion()
		{
			...
			if (m_region.Coords != null && m_region.Coords.Count > 0)
			{	// we have coords
				if (m_region.Coords[0] is Rectangle2D)
				{	// we have a rect
					Rectangle2D zone = (Rectangle2D)m_region.Coords[0];
					Region begin = Region.Find(new Point3D(zone.Start, 0), base.Map, m_region);
					Region end = Region.Find(new Point3D(zone.End, 0), base.Map, m_region);
					if (begin == end)
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			for (int ix = 0; ix < m_region.Coords.Count; ix++)
				writer.Write((Rectangle2D)m_region.Coords[ix]);
		}
				...
				case 0:
					{
						...
						{
							Rectangle2D rx = reader.ReadRect2D();
		...
		public class OnPlacementTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targ)
			{
				...
				{
					m_SensoryConsole.Begin = new Point2D((targ as IPoint3D).X, (targ as IPoint3D).Y);
					from.Target = new OnPlacementTarget(m_SensoryConsole, false);
					from.SendMessage("Target the lower right corner of the new zone.");
				}
				else			// second point
				{
					m_SensoryConsole.End = new Point2D((targ as IPoint3D).X, (targ as IPoint3D).Y);
...
namespace Server.Items
{
	public class SensoryZoneDeed : Item
	{
		...
		public static bool CanPlaceNewSensoryZone(Multis.BaseHouse house)
		{
			...
			{
				if (rx is SensoryRegion && rx.Coords != null)
					for (int ix = 0; ix < rx.Coords.Count; ix++)
					{
						if (rx.Coords[ix] is Rectangle2D)
							if (house.Contains(((Rectangle2D)rx.Coords[ix]).Start))
								count++;
					}
			}
