	...
	public partial class CTFControl : Server.Items.RegionControl
	{
		public void OnDragDrop(Mobile broker, Mobile from, Item dropped)
		{
			...
			if (MobileOk(Captain1) && Captain1 == from && CurrentState > States.Quiescent)
			{
				...
				for (int ix = 0; ix < DefaultsSpellConfig.Count; ix++)
					this.RestrictedSpells[ix] = DefaultsSpellConfig[ix];
		...
		{
			return new Server.Engines.CTFRegion(rc, map);
		}
		...
		{
			ArrayList regions = Region.FindAll(location, map);
			if (regions != null)
			{
				for (int ix = 0; ix < regions.Count; ix++)
				{
					Region rx = regions[ix] as Region;
					if (rx.UId == this.MyRegion.UId)
						return true;
				}
			}
		...
		{
			// set the range
			if (this.MyRegion != null && this.MyRegion.Coords != null && this.MyRegion.Coords[0] != null)
			{
				// kill old arrow
		...
		protected bool AllowMageryImp
		{
			...
			{	// if any spells are allowed, the magery is said to be allowd
				for (int ix = 0; ix < this.RestrictedSpells.Count; ix++)
					if (!this.RestrictedSpells[ix])
						return true;
				return false;
			}
			set
			{
				// turn magery either all on, or all off
				for (int ix = 0; ix < this.RestrictedSpells.Count; ix++)
					this.RestrictedSpells[ix] = !value;
			}
		}
