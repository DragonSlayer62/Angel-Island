	...
	public class ReportMurdererGump : Gump
	{
		...
		private void BuildGump()
		{
			...
			else
			{
				...
				string text = String.Format("Would you like to report {0} as a murderer?", ((Mobile)m_Killers[m_Idx]).Name);
				text = String.Format("<font face=\"Comic Sans MS\">{0}</font>", text);
				AddHtml(x + 63, y + 51, 266, 110, text, false, false);
