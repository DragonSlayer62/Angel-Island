...
namespace Server.Items
{
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable/*, ISlayer, IDurability*/
	{
		...
		*/
		...
		public virtual int GetPackInstinctBonus(Mobile attacker, Mobile defender)
		{
			...
			foreach (Mobile m in eable)
			{
				if (m != attacker && m is BaseCreature)
				{
					...
					if (master == theirMaster && tc.Combatant == defender && Utility.RandomChance((int)tc.Loyalty))
						++inPack;
				}
			}
			eable.Free();
		...
		{
			// will always be false is Core.AngelIsland so no need to condition it here
		...
		public virtual void OnHit(Mobile attacker, Mobile defender)
		{
			...
			bool good =
				...
				!(defender as BaseCreature).ControlMaster.Murderer)		// and my master is not a murderer
				? true : false;
						...
						case CraftResource.Valorite:
							{
								Identified = true;
								DurabilityLevel = WeaponDurabilityLevel.Indestructible;
								DamageLevel = WeaponDamageLevel.Vanquishing;
