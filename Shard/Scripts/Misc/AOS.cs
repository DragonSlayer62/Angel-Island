...
namespace Server
{
	public class AOS
	{
		public static void Configure()
		{
			...
			//The below line is the default distribution setting
			//SupportedFeatures.Value = ( Enabled ? 0x801F : 0x3 );
			int iSupportedFeatures = 0;
			iSupportedFeatures += 0x01; //T2A stuff (?)
			iSupportedFeatures += 0x02; //chat
			iSupportedFeatures += 0x04; //don't know about this bit
			iSupportedFeatures += 0x08; //LBR animations
			iSupportedFeatures += 0x10; //AOS animations need this to be one

			iSupportedFeatures += 0x40; //SE stuff
			iSupportedFeatures += 0x80; //ML stuff
			iSupportedFeatures += 0x100; //8th age stuff

			iSupportedFeatures += 0x8000; //AOS animations need this to be one
