...
namespace Server
{
	public class VendorGenerator
	{
		...
		private static bool GetFloorZ(Map map, int x, int y, out int z)
		{
			...
			if (IsFloor(lt.ID) && map.CanFit(x, y, lt.Z, 16, CanFitFlags.requireSurface))
			{
				z = lt.Z;
				return true;
			}
			...
			for (int i = 0; i < tiles.Length; ++i)
			{
				...
				if (IsStaticFloor(t.ID) && map.CanFit(x, y, t.Z + (id.Surface ? id.CalcHeight : 0), 16, CanFitFlags.requireSurface))
				{
