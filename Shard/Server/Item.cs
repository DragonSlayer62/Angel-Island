 ...
 * CHANGELOG:
 *	3/16/16, Adam
 *		Add support for labels of the form: "a bread loa%ves/f%"
 *			From this we generate "bread loaves" or "a bread loaf"
 *	8/27/11, Adam
 *		Update out custom labeling system to support our new custom Cliloc.
 *		We process plural specifically based on custom range
 *		i.e., if (useLabel && LabelNumber >= 7981 && LabelNumber <= 8044)
 *	9/11/10, adam
 *		Add item creation date.
 *		We have needed something like this over the years and I've finally decided it's worth the data size increase.
 *		Our first place where this will help us is in the SeaChart. SeaCharts are constructed from a map database and we need a 
 *		creation date so that we will know from which version of the map database the seachart was generated.
 *	5/12/09, Adam
 ...
 *
 *   $Id: Item.cs,v 1.97 2011/09/11 14:58:55 luket Exp $
 *   $Author: luket $
 *   $Date: 2011/09/11 14:58:55 $
 *
 *
 ***************************************************************************/
...
namespace Server
{
	...
	public class Item : IPoint3D, IEntity, IHued
	{
		...
		#region Spawner Location
		Point3D m_SpawnerLocation = Point3D.Zero;
		Map m_SpawnerMap = null;

		[CommandProperty(AccessLevel.GameMaster)]
		public Point3D SpawnerLocation
		{
			get { return m_SpawnerLocation; }
			set { m_SpawnerLocation = value; }
		}
		public Map SpawnerMap
		{
			get { return m_SpawnerMap; }
			set { m_SpawnerMap = value; }
		}
		#endregion
		...
		*/
		...
		private enum SaveFlag
		{
			...
			SpawnerLocation = 0x20000000	// adam: location of spawner that spawned this item (Point3D.Zero for no spawner)
		}
		...
		public virtual void Serialize(GenericWriter writer)
		{
			...
			if (m_SpawnerLocation != Point3D.Zero) flags |= SaveFlag.SpawnerLocation;
			...
			// version 12
			if (GetSaveFlag(flags, SaveFlag.SpawnerLocation))
			{
				writer.Write(m_SpawnerLocation);
				...
				case 12:
					{
						if (GetSaveFlag(flags, SaveFlag.SpawnerLocation))
						{
							m_SpawnerLocation = reader.ReadPoint3D();
							m_SpawnerMap = reader.ReadMap();
						}
						goto case 11;
					}
				case 11:
					{
						...
						if (version == 11 && GetSaveFlag(flags, SaveFlag.SpawnerLocation))
						{
							m_SpawnerLocation = reader.ReadPoint3D();
							m_SpawnerMap = reader.ReadMap();
						}
		...
		// TODO: make Name non-virtual and use DefaultName as per RunUO
		[CommandProperty(AccessLevel.GameMaster)]
		...
		public virtual void OnBeforeFreezeDry()
		{
		}
