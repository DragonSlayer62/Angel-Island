...
/* Server/JobManager.cs
 * Changelog
 *  5/6/08, Adam
 *		Turned off taran's thread manager as we're not using it and it makes profiling a bit tougher
 *  6/13/07, Adam
 *      Change the Sleep (1) to Sleep(100)
 *      We're having 'startup' timing issues and I want to reduce this service's CPU hunger
 *      I don't see wht the scheduler needs to pump that fast anyway
 *  11/20/06 Taran Kain
 *      Changed some of the try/catch return values. Still investigating timeouts.
 *  11/19/06, Adam
 *      We've been suffering many unhandled LockTimeoutExceptions since we moved to .Net 2.0.
 *      I do not believe the problem is 2.0 per say as we used to get get the exceptions when the 
 *      server was backing up (NTBACKUP). I believe that all that has happened is that the .NET 2.0 has simply 
 *      shifted the point in the code where the timeout occures.
 *      To this end, I have wrapped ALL calls to TimedLock.Lock(SyncRoot) with a 'generic' exception.
 *      I chose the generic exception to clearly show that this was both a new addition, 
 *      AND more importantly, and unexpected exception.
 *      Also: added Error constant to the ThreadStatus enum to reflect a timeout error.
 *	05/19/06, Adam
 *		avoid exception by explicitly handling case
 *		In effect: if (m_Completed == null || m_Completed.Count() == 0) return;
 *  02/24/06 Taran Kain
 *      Change JM.JobScheduler to Sleep(1), increase granularity but keep cpu normal
 *	02/17/06, Adam
 *		1. Reduce the scheduler priority to: ThreadPriority.BelowNormal;
 *		2. Add back the sleep(X) until we make the Scheduler interrupt driven
 *  02/14/06 Taran Kain
 *		Added a few try/finally sanity-check blocks.
 *		Named worker threads.
 *		Changed ThreadObject to disallow resetting ThreadStatus when Status == Dead, prevents race condition
 *		Changed Monitor.Pulse/Wait into AutoResetEvent, prevents race condition
 *  01/23/06 Taran Kain
 *		Rebuilt thread communication model. All communication now occurs through ThreadObject, and ThreadScheduler() handles almost all housekeeping.
 *		Removed many locks :)
 *  01/18/06 Taran Kain
 *		Fixed minor bug in one of ThreadJob's ctors not setting m_Status
 *		Removed two deadlock possibilities
 *		Added /// comments to more easily see who locks what where
 *		Switched over to TimedLock implementation instead of lock(), infinite deadlocks are now a thing of the past
 *	01/03/06 Taran Kain
 *		Added LastError getter, to prevent spamming console. Now only prints when new error != LastError
 *  01/01/06 Taran Kain
 *		Removed possibility of deadlock condition
 *		Added JobStatus enum
 *		Added TotalJobsRun, TotalJobTime, QueueLengthLimit
 *		JobManager queue size is now limited to 128 jobs by default
 *		Removed debug console messages.
 *  12/29/05 Taran Kain
 *		Cleaned up thread die-off, fixed some sync issues
 *	12/29/05 Taran Kain
 *		Initial version
 */
