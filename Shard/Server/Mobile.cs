 ...
 * CHANGELOG:
 *	3/15/16, Adam,
 *		When players were squelched, they would see this message when thay speak: "You can not say anything, you have been squelched."
 *		Now we play a dirty trick on them. they don't know that are squelched, just let them think everyone can hear them, but are ignoring them.
 *		nearby staff will see what is said though, like this: "Squelched: the spoken text"
 *	2/16/11, Adam
 *		In OnBeneficialAction(), check is UOSP || UOMO and either generate a criminalAction(guard whack) or simply make the healer criminal
 *		UOMO Red Healing is a criminal offence but will not get you Guard whacked 
 *	2/10/11, adam
 *		In OnBeneficialAction(), check is UOSP and either generate a criminalAction(guard whack) or simply make the healer criminal
 *		UOSP Red Healing is a criminal offence but will not get you Guard whacked 
 *	8/18/10, adam
 *		Add a generic OnEvent handler so AIs and leaf classes can define non specific events
 *		For an example, see the OnEvent handler in TaxCollectorAI.cs and TaxCollector.cs
 *	7/9/10, adam
 *		Add a virtual InitClass() function to help in defining derived mobile classes. For instance:
 *		The class Pirate is the Base for Deck Hands, Pirate Champs, Pirate Wenches, and the Pirate Captian.
 *		The InitBody, InitClass and InitOutfit clearly define those components of the mobile that are either shared or private
 *		InitClass defines those things private to the class like str/hits/virtual armor (usually found in the constructor)
 *	6/21/10, adam
 *		Add a Remembers() function. This is implemented in the BaseCreature and is used to remember players that may have committed a crime.
 *	6/10/10, adam
 *		Add new BlockDamage property that can be set at runtime to block damage to a player.
 *		This is never serialized and is better than blessed for certain kinds of damage testing.
 *	5/17/10, adam
 *		o Save/Restore the time remaining left in the criminal timer
 *			We do this because criminal timers (PJUM for instance) can now be longer than 2 minutes
 *		o enhance ExpireCriminalTimer to take a TimeSpan argument
 *		o Add new MakeCriminal(TimeSpan) function to make a player a criminal for however long
 *	5/7/10, Adam
 *		Add LastStableChargeTime so that we can actually charge the player for stabled pets
 *		Add SaveFlags to optimize the save/load times
 *	4/10/10, Adam
 *		Added Region.OnAfterDeath(this) call so we can paralyze ghosts via the region since the Frozen flag 
 *		is cleared AFTER the Region.OnDeath() call
 *	3/6/10, Adam
 *		set the corpse container in OnDeath.
 *		We look at this when creating a head from a corpse for bounty purposes.
 *	1/7/09, Adam
 ...
 *
 *   $Id: Mobile.cs,v 1.141 2011/08/19 15:18:41 luket Exp $
 *   $Author: luket $
 *   $Date: 2011/08/19 15:18:41 $
 *
 *
 ***************************************************************************/
...
namespace Server
{
	...
	public class Mobile : IEntity, IPoint3D, IHued
	{
		...
		{	// non notority flags
			...
			// 75 on reserved Notority flags (sends notority update packets)
			BeginNotoFlags = 75,
		...
		public class ExpirationFlag
		{
			...
			public ExpirationFlag(Mobile mobile, ExpirationFlagID state, DateTime start, TimeSpan span)
			{
				...
				{	// force the client to recognize the new notority
		...
		/// <summary>
		/// Check access of caller and allow if appropriate acces and print a message
		/// </summary>
		...
		*/
		...
		{
			if (!GetFlag(MobileFlags.SuicideBomber))
			{
				SuicideBomber = true;
		...
		public bool SuicideBomber
		{
			get
			{
				return GetFlag(MobileFlags.SuicideBomber);
			}
			set
			{
				if (GetFlag(MobileFlags.SuicideBomber) != value)
				{
					SetFlag(MobileFlags.SuicideBomber, value);
		...
		{
			if (!GetFlag(MobileFlags.InvisibleShield))
			{
		...
		public bool InvisibleShield
		{
			get
			{
				return GetFlag(MobileFlags.InvisibleShield);
			}
			set
			{
				if (GetFlag(MobileFlags.InvisibleShield) != value)
				{
					SetFlag(MobileFlags.InvisibleShield, value);

					this.SendMessage(GetFlag(MobileFlags.InvisibleShield)
...
#else
			// adam: the problem with pix's logic here is that on server up there
			//	will me no matched EndAction() to clear the lock
			//	It's better 
#endif
		}
		...
		public AccessLevel AccessLevel
		{
			...
			set
			{
				...
				if (value == AccessLevel.Owner) return;

				AccessLevelInternal = value;
			}
		}

		// do not make a command, this is fro programatic use only.
		...
		public virtual void OnBeneficialAction(Mobile target, bool isCriminal)
		{
			if (isCriminal)
			{
				...
				{	// Healing a red character is a criminal offence, but will not get you guard whacked.
					...
					else if (target.Evil)
					{
						...
						// they are just red, so make the healer crim
						Criminal = true;
				}
			}
			...
			// BeneficialActions on Evils doesn't seem to fit in with the normal notority system
		...
		[CommandProperty(AccessLevel.Administrator, AccessLevel.Administrator)]
		public virtual bool IsInvulnerable { get { return false; } set { } }
