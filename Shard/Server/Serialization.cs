 ...
 *
 *   $Id: Serialization.cs,v 1.23 2014/03/26 15:55:31 luket Exp $
 *   $Author: luket $
 *   $Date: 2014/03/26 15:55:31 $
 *
 *
 ***************************************************************************/
...
namespace Server
{
    ...
    /// <summary>
    /// We no longer call out to our secret dongle to manage encryption of the database, but do it inline here.
    /// Originally, we had the encryption dongle time-out every year to prevent a rogue developer from absconding
    /// with our code. The dongle timeout would auto-bork their database after the timeout. 
    /// We still encrypt the database here, but for simple backwards compatibility. 
    /// </summary>
	public class Pack32
	{
		/* software dongle no longer needed for open source */
		/* turn on for Angel Island */
		...
		{
			if (Server.Core.Import && pack == false)
				return value;

            // old code
			//int nvalue = iPack32(value, pack);
			//return nvalue;

            // new code
            uint uvalue = (uint)value;
            uint mask = 0xDeadBeef;
            if (pack == true)
            {
                uvalue = uvalue ^ mask;
                uvalue = (uvalue << 13) | (uvalue >> (32 - 13));
            }
            else //(pack == false)
            { 
                uvalue = (uvalue >> 13) | (uvalue << (32 - 13));
		...
		public static uint _uPack32(uint value, bool pack)
		{
            ...
            // old code
			//uint nvalue = uPack32(value, pack);
			//return nvalue;
